...
try {
    ...
    $sql = "CREATE TABLE IF NOT EXISTS appliances (
        ...
        image_path VARCHAR(255) DEFAULT NULL, -- Optional, kept for completeness though user didn't ask explicitly yet
    ...
    $user = $stmtUser->fetch();
    $userId = $user ? $user['id'] : 1; // Fallback to 1
    ...
    if ($stmtCount->fetchColumn() == 0) {
        ...
        $appliances = [
            ...
            [$userId, 'Nồi cơm điện Sharp', 'Khác', 70000, 0, 'Nồi cơm nắp gài 1.8L', 'available'] // 0 quantity example
        ];
